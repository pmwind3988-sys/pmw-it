...
// on.
//
// The series palette is read the same way, from `--ds-series-1..8` in
// datastudio.css, which is where its colour-blindness reasoning lives.
...
export function seriesPalette() {
  return Array.from({ length: SERIES_SLOTS }, (_, i) => token(`--ds-series-${i + 1}`))
    .filter(Boolean);
}
