...
/**
 * Which columns the row TABLE shows, in a readable order.
 *
 * The full record card shows everything; the table cannot, or it is a
 * horizontal scrollbar with nothing on screen. Columns the autopilot
 * parked as bookkeeping sort last rather than being dropped, so a sheet
 * where the autopilot got it wrong still shows something recognisable
 * once the user widens the table.
 */
export function tableColumns(dataset, limit = 8) {
  if (!dataset) return [];
  const ranked = dataset.columns
    .map((column, index) => ({ column, index }))
    .sort((a, b) => {
      const ai = a.column.role === 'ignored' ? 1 : 0;
      const bi = b.column.role === 'ignored' ? 1 : 0;
      if (ai !== bi) return ai - bi;
      return a.index - b.index;
    });
  return (limit === null ? ranked : ranked.slice(0, limit)).map((r) => r.column);
}
