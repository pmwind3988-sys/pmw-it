...
describe('planAutopilot', () => {
  ...
  it('starts the analysis by itself when the title is about writing', () => {
    expect(plan.autoAnalyse).toBe(true);
  });
  ...
  it('offers but does not start the analysis on a sheet that is not about writing', () => {
    ...
    expect(stock.autoAnalyse).toBe(false);
  });
  ...
  it('survives an import with no profile at all', () => {
    ...
    expect(result.autoAnalyse).toBe(false);
  });
});
