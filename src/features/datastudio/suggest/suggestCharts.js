...
function tile(id, title, chart, encoding, extra = {}) {
  return {
    ...
    size: 'M',
    stacked: false,
    // Tiles respond to cross-filters by default; the tile editor is
    // where a user opts one out (a KPI showing the unfiltered total, for
    // instance).
...
export function suggestCharts(profile, dataset = null) {
  ...
  const chartableDimensions = dimensions.filter(
    (c) => c.distinctCount <= BAR_CARDINALITY_LIMIT,
  );
