...
export default function ChartTile({ tile, dataset, mask, selection, onSelect, onChartInit }) {
  ...
  if (!validity.ok) {
    return (
      <div className="sa-tile-body sa-tile-broken" role="status">
        <p>{validity.reason}</p>
        <p className="sa-tile-broken-hint">
          The column it was drawn from is no longer in the sheet. Remove this
          chart, or drop the file again.
        </p>
      </div>
    );
  }
  ...
  if (result.categories.length === 0 && tile.chart !== 'kpi') {
    return (
      <div className="sa-tile-body sa-tile-empty" role="status">
  ...
  if (option.kind === 'kpi') {
    return (
      <div className="sa-tile-body sa-kpi">
        <span className="sa-kpi-value">{formatValue(option.value, tile.isPercent)}</span>
        <span className="sa-kpi-label">{option.label}</span>
      </div>
    );
  }
  ...
  if (option.kind === 'table') {
    return (
      <div className="sa-tile-body sa-tile-table-scroll">
        <table className="sa-table">
          <thead>
            <tr>
              {option.headers.map((h) => (
                <th key={h} className={h === option.headers[0] ? undefined : 'sa-num'}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {option.rows.map((row) => (
              <tr key={String(row[0])}>
                {row.map((cell, i) => (
                  <td key={`${row[0]}-${i}`} className={i === 0 ? undefined : 'sa-num'}>
  ...
  return (
    <EChart
      className="sa-tile-body sa-tile-chart"
      option={option}
      onEvents={onEvents}
      onInit={handleInit}
    />
