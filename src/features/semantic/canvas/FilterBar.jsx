...
export default function FilterBar() {
  ...
  return (
    <div className="sa-filterbar">
      <Filter size={14} className="sa-filterbar-icon" />

      {selection && (
        <span className="sa-chip sa-chip-selection">
          <span>
      ...
      {globalFilters.map((filter) => (
        <span className="sa-chip" key={filter.column}>
      ...
      <label className="sa-filterbar-add">
        <span className="sa-sr-only">Add a filter</span>
        <select
          className="sa-select"
          value={pendingColumn}
      ...
      {pending && (
        <label className="sa-filterbar-add">
          <span className="sa-sr-only">{`Value for ${pending.name}`}</span>
          <select
            className="sa-select"
            value=""
            onChange={(e) => {
