...
describe('detectAdminColumns', () => {
  ...
  it("catches the respondent's name, without catching a question about a name", () => {
    expect(hidden(['Name', 'Full name', 'Name of the machine that failed']))
      .toEqual(['Name', 'Full name']);
  });

  it('catches a bare row id', () => {
    expect(hidden(['Id', 'Identify the root cause'])).toEqual(['Id']);
  });
