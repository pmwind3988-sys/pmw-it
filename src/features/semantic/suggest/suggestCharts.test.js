...
describe('suggestCharts', () => {
  ...
  // Twelve written answers squeeze under the cardinality limit, so
  // width alone does not save a small sheet from a bar chart whose x
  // axis is twelve paragraphs.
  it('never charts a column named as a written answer, however few there are', () => {
    const written = {
      headers: ['Department', 'Describe the biggest issue you face'],
      rows: Array.from({ length: 12 }, (_, i) => [
        i % 2 ? 'HR' : 'Finance',
        `Sentence number ${i} about something that went wrong at work today.`,
      ]),
    };
    const profile = profileDataset(written);
    const answer = 'Describe the biggest issue you face';

    // Without being told, it charts them: a dozen distinct sentences
    // profile as an ordinary categorical column of a dozen values.
    expect(suggestCharts(profile).some((x) => x.encoding.x?.column === answer)).toBe(true);

    const t = suggestCharts(profile, null, [], [answer]);
    expect(t.some((x) => x.encoding.x?.column === answer)).toBe(false);
    expect(t.some((x) => x.encoding.x?.column === 'Department')).toBe(true);
  });
