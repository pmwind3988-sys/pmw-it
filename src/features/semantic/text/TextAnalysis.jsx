  ...
  return (
    <>
      ...
      <div className="sa-toolbar">
        ...
        {/* The charts were built from this reading the moment it
            landed. This rebuilds them from the reading AS CORRECTED,
        ...
        <Button
          ...
          disabled={!analysis}
        >
          Chart my corrections
        </Button>
        <Button variant="ghost" size="sm" icon={RefreshCw} onClick={() => setStage('dashboard')}>
          Back to charts
        </Button>
      </div>
      ...
      {analysing && (
        <Card>
          <p className="sa-summary">{textProgress.stage || 'Working'}</p>
      ...
      {analysis && (
        <>
          ...
          <div className="sa-text-tabs" role="tablist">
            {VIEWS.map(([id, label]) => (
              <button
                key={id}
                type="button"
                ...
                className={`sa-text-tab${view === id ? ' sa-text-tab-on' : ''}`}
                onClick={() => setView(id)}
              >
                {label}
              </button>
